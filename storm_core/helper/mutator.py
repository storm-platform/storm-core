...
class GraphMutator:
    ...
    @staticmethod
    def mutate_graph_to_execution_plan_by_outdated_compendia(graph, outdated_compendia, output_directory):
        ...
        for outdated_compendium in outdated_compendia:
            command_job = CommandJob(outdated_compendium.command, output_directory, outdated_compendium.name)
        ...
        for vertex_src in outdated_graph.vs:
            ...
            vertex_src_original = graph.vs.select(name=vertex_src["name"])[0]  # should exists!
            ...
            for vertex_predecessor_idx in graph.predecessors(vertex_src_original):
                ...
                vertex_predecessor_outdated = outdated_graph.vs.select(name=vertex_predecessor["name"])
    ...
    @staticmethod
    def mutate_index_graph_to_compendia_job_graph(graph, output_directory, current_compendia) -> ExecutionPlan:
        ...
        for original_vertex in graph.vs:
            ...
            for vertex_predecessor in original_vertex.predecessors():
                job_vertex_predecessor = job_graph.vs.select(name=vertex_predecessor["name"])[0]
